...
static BOOLEAN IsCodePadding(LPBYTE pInst, UINT size) {
    UINT i;
...
BOOLEAN CreateTrampolineFunction(PTRAMPOLINE ct) {
    ...
    UINT8     tempTrampoline[MEMORY_SLOT_SIZE];
    ...
    ct->patchAbove = FALSE;
    ct->nIP = 0;
    RtlZeroMemory(tempTrampoline, MEMORY_SLOT_SIZE);
    ...
    __try {
        do {
            ...
            copySize = HDE_DISASM((PVOID)pOldInst, &hs);
            if (hs.flags & F_ERROR)
                return FALSE;
            ...
            if (oldPos >= sizeof(JMP_REL)) {
                ...
                RtlCopyMemory(instBuf, (LPBYTE)pOldInst, copySize);
                pCopySrc = instBuf;
            ...
            RtlCopyMemory((LPBYTE)tempTrampoline + newPos, pCopySrc, copySize);
            newPos += copySize;
            oldPos += hs.len;
        } while (!finished);
    }
    ...
    if (oldPos < sizeof(JMP_REL) && !IsCodePadding((LPBYTE)ct->pTarget + oldPos, sizeof(JMP_REL) - oldPos)) {
        ...
        if (!IsExecutableAddress((PVOID)((LPBYTE)ct->pTarget - sizeof(JMP_REL))))
            return FALSE;
    ...
    RtlCopyMemory((LPBYTE)tempTrampoline + newPos, &jmp, sizeof(jmp));
    newPos += sizeof(jmp);
#endif

    PMDL mdl = IoAllocateMdl(ct->pTrampoline, newPos, FALSE, FALSE, NULL);
    if (mdl) {
        __try {
            ...
            if (mapping) {
                ...
                if (NT_SUCCESS(status)) {
                    RtlCopyMemory(mapping, tempTrampoline, newPos);
                }
